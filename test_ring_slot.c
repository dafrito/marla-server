...
{
    int CAP = 8;
    parsegraph_Ring* ring = parsegraph_Ring_create(CAP);
    ...
    void* buf;
    size_t len;
    parsegraph_Ring_writeSlot(ring, &buf, &len);
    ...
    if(parsegraph_Ring_size(ring) != 8) {
        fprintf(stderr, "Ring size(%d) is unexpected.\n", parsegraph_Ring_size(ring));
        return 1;
    }

    parsegraph_Ring_readSlot(ring, &buf, &len);
    if(len != 8) {
